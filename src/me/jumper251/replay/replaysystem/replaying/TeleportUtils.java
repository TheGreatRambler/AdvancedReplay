...
public class TeleportUtils {
	...
	static {
		...
		try {
			...
			position = entity.getDeclaredMethod("setLocation", Double.TYPE,
				Double.TYPE, Double.TYPE, Float.TYPE, Float.TYPE);
			...
			packetConstructor = packetClass.getConstructor(Double.TYPE,
				Double.TYPE, Double.TYPE, Float.TYPE, Float.TYPE, Set.class,
				Integer.TYPE);
			vec3D             = vecClass.getConstructor(
                Double.TYPE, Double.TYPE, Double.TYPE);

			Object[] enumObjects
				= getNmsClass("PacketPlayOutPosition$EnumPlayerTeleportFlags")
					  .getEnumConstants();
			teleportFlags = Collections.unmodifiableSet(
				new HashSet<>(Arrays.asList(enumObjects[4], enumObjects[3])));
	...
	public static void teleport(Player player, Location location) {
		...
		try {
			if(activeContainer.get(handle) != defaultContainer.get(handle))
				closeInventory.invoke(handle);
			position.invoke(
				handle, x, y, z, yaw.get(handle), pitch.get(handle));
			...
			Object packet = packetConstructor.newInstance(
				x, y, z, 0, 0, teleportFlags, teleportAwait);
	...
	private static String getVersion() {
		return Bukkit.getServer().getClass().getPackage().getName().substring(
			23);
	}
}